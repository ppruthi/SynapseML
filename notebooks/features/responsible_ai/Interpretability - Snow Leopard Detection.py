...
def bingPhotoSearch(name, queries, pages):
  ...
  return spark.createDataFrame(parameters, ("queries","offsets"))     .mlTransform(
...
randomLinks = randomWords   .mlTransform(BingImageSearch()
...
images = snowLeopardUrls.union(randomLinks).distinct().repartition(100)  .mlTransform(BingImageSearch.downloadFromUrls("urls", "image", concurrency=5, timeout=5000))  .dropna()
...
lime = ImageLIME()  .setModel(fitModel)  .setPredictionCol("leopard_prob")  .setOutputCol("weights")  .setInputCol("image")  .setCellSize(100.0)  .setModifier(50.0)  .setNSamples(300)
