...
model = CNTKModel()     .setModelLocation(modelSchema.uri)     .setInputCol("features")     .setOutputCol("probs")     .setOutputNodeIndex(0)     .setMiniBatchSize(1)
...
df = df.withColumn("formattedSentence", prettyPrintUDF("tokens", "entities"))        .select("formattedSentence")
